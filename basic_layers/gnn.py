...
class GNN(KL.Layer):
    ...
    def node_classify(self, node_states, training):
        node_classic = self.node_classify_net(node_states, training=training)
        if self.gated:
            s0, s1 = tf.split(
                node_classic, num_or_size_splits=2, axis=1)
            gates = tf.nn.softmax(s0)
            node_classic = gates * s1
        return node_classic
